...
func (c *Conn) handleNextCommand(handler Handler) error {
	...
	case ComQuery:
		...
		c.recycleReadPacket()
		if c.IsFirstQuery && c.IsPymysqlNewVersion {
			log.Info("ignore first packet in pymysql 1.1.X")
			c.writeOKPacket(12, 34, 56, 78)
			c.flush()
			c.IsFirstQuery = false
			return nil
		}
