...
func (db *DB) NewConnection(c *mysql.Conn) {
	...
	if c.ConnAttrs != nil {
		...
		log.Info("===============")
		if _, existsClientName := c.ConnAttrs["_client_name"]; existsClientName {
			if version, existsClientVersion := c.ConnAttrs["_client_version"]; existsClientVersion {
				if strings.HasPrefix(version, "1.1") {
					c.IsPymysqlNewVersion = true
					c.IsFirstQuery = true
				}
			}
		}
